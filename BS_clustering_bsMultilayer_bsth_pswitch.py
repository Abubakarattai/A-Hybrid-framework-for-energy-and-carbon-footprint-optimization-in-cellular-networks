...
def power_module(input_index, row, traffic):
    ...
    #print('print psaved', p_bs_off+p_bs_on)
        
    #print('print power', P_all_on-(p_bs_off+p_bs_on))
    
    #print('power all', P_all_on)
...
def multi_cluster(cluster_and_traffic_df, row, x):
    ...
    if len(cluster_df) > 0:
        ...
        for kQ in cluster_keys:
            ...
            #print('cluster to remove', (cluster_remove))
            #print('total traffic', total_traffic)
            for index_i, i in enumerate(cluster_remove):
                #print('i    ', i)
                #print(storage_energy)
                power_on_all = power_module(i, row, total_traffic[index_i])
                traffic_index.append(i)
                storage_energy.append(power_on_all[0])
                storage_energy2.append(power_on_all[2])
                #storage_energy3.append(power_on_all[3])
    ...
    if (len(cluster_and_traffic_df) == 0) :
        ...
        #print('storage', len(storage_energy))
        #print('argmax', np.argmax(np.array(storage_energy)))
        #indexmax = np.argmax(np.array(storage_energy))
        indexmax = np.argmin(np.array(storage_energy2))
        #print('utilization', storage_energy[indexmax])
        #print('utilization2', storage_energy2[indexmax])
        #print('min ', min(storage_energy2))
        #print('combination', len(traffic_index))
        #print('combination',traffic_index[indexmax])
        
        return max(storage_energy), min(storage_energy2), traffic_index[indexmax],power_on_all[3]
...
for n_sc in tqdm([8, 12, 16, 20, 24, 36, 48, 60, 72, 84, 96, 108, 120]):
#for n_sc in tqdm([8]):
#for n_sc in [96]:
    ...
    for path_sc in os.listdir(file_name_sc):
        ...
        rows = 144     # change it later
        
        #file = pathlib.Path('results/databsth_pswitch_'+str(n_sc)+'.csv' )  
        #if file.exists ():
        #    df_n_sc= pd.read_csv('results/databsth_pswitch_'+str(n_sc)+'.csv' )
        #else:
        ...
        #for row in [102]:
            ...
            output_info1, output_info2, output_info3, output_info4  = multi_cluster(cluster_and_traffic_df, row,  2)
            #print('row', row)
            #print('info 1', output_info1)
            #print('info 2', output_info2)
            #print('info 3', output_info3)
            #print('info 4', output_info4)
            yyy = [1 for iii in range(n_sc)]
            for xxx in output_info3:
                if xxx != -1:
                    yyy[xxx] = 0

            #print(multi_cluster(cluster_and_traffic_df, row,  2))
            #storage_energy_total.append(output_info1)
            df_list = Tf.loc[row].values.tolist()
            for xxx in yyy:    
                df_list.append(xxx)
            df_list.append(output_info1)
            df_list.append(output_info2)
            df_list.append(output_info4)
            
            df_list = pd.DataFrame(df_list).T
            
            newdata = newdata.append(df_list)
            #newdata.loc[row] = df_list
            
            #newdata.iloc[row] = df_list
            #print(traffic_sc2.iloc[row])
            #print()
        ...
        #df_n_sc.to_csv('results/databsth_pswitch_'+str(n_sc)+'.csv', index=False)
            
        newdata.to_csv('pmulti_bsth/clust_data_'+str(n_sc)+'/'+path_sc, index=False, header= None) 
    ...
    #     for i in bs_number:
    #         file = pathlib.Path('results/databsth_pswitch_'+str(i)+'.csv')
    #         if file.exists ():
    #             #print(i)
    #             df_n_sc= pd.read_csv('results/databsth_pswitch_'+str(i)+'.csv')
    ...
    #     out.to_csv('out/output_alldatabsth_pswitch.csv') 
    #     #print(full_path)
